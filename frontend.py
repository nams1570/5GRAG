...
def TogglePrompt():
    IS_PT = langchain_controller.toggleRetriever()
    if IS_PT:
        text = "MULTI QUERY PROMPT ENABLED"
    else:
        text = "MULTI QUERY PROMPT DISABLED"
    return text
...
with gr.Blocks() as demo:
    ...
    togglePT = gr.Button("MULTI QUERY PROMPT DISABLED")
    togglePT.click(fn=TogglePrompt,outputs = togglePT)
    chatbot=gr.Chatbot(height=500)
    textbox=gr.Textbox(placeholder="Ask me any question", container=False, scale=7)
